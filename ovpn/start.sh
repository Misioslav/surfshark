...
then
	ls -l /ovpn/ovpn-servers/*udp.ovpn | awk -F'[/.]' '{print "remote " $4"."$5"."$6".com 1194 udp"}' >> /ovpn/servers2.txt
else
	ls -l /ovpn/ovpn-servers/$LOCATION*udp.ovpn | awk -F'[/.]' '{print "remote " $4"."$5"."$6".com 1194 udp"}' >> /ovpn/servers2.txt
fi
